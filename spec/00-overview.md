...
The project is no longer in the "initial implementation" stage. It now has a
working WebGPU app, model parsing, maneuver logic, smoke generation, and a
recursive field loader. However, it is **not yet a full behavior-equivalent
port** of the original screensaver.
...
| OpenGL backend (iopengl.c) | `src/renderer.ts` + `src/shader.wgsl` | Rewrite for WebGPU |
...
| `src/shader.wgsl` | 150 | WGSL vertex/fragment shaders |
...
After each implementation step, run `npm run dev` and visually verify in browser:
...
Final: Run `npm run build` to verify production build succeeds.
