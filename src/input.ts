...
export function clampCameraPitch(angle: number): number {
  return Math.max(-CAMERA_PAN_PITCH_LIMIT, Math.min(CAMERA_PAN_PITCH_LIMIT, angle));
}
...
export function registerInputHandler(state: AppState): void {
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "d" || e.key === "D") {
      ...
      state.debugOverlay.parentElement?.classList.toggle(
        "is-debug-hidden",
        !state.debugOverlayVisible
      );
      return;
    }
...
export class GameplayInputController {
  ...
  sample(kind: VehicleKind): VehicleCommand {
    ...
    const steerKeys =
      (keyActive(this.keys, "KeyD", "ArrowRight") ? 1 : 0) -
    ...
    const rollKeys =
      (keyActive(this.keys, "KeyD") ? 1 : 0) - (keyActive(this.keys, "KeyA") ? 1 : 0);
    const pitchKeys =
      (keyActive(this.keys, "ArrowDown") ? 1 : 0) - (keyActive(this.keys, "ArrowUp") ? 1 : 0);
    const yawKeys = (keyActive(this.keys, "KeyE") ? 1 : 0) - (keyActive(this.keys, "KeyQ") ? 1 : 0);
